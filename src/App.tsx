...
function AppContent() {
  ...
  const { workspaces, currentWorkspace, createWorkspace, addFolderToWorkspace } = useWorkspace()

  // Add a new state for accent color
  const [accentColor, setAccentColor] = useState<"cyan" | "crimson" | "purple" | "green" | "gray">("cyan")
  ...
  return (
    ...
    >
      ...
      <header className={`border-b border-border ${theme === "dark" ? "bg-[#0A0A0A]" : "bg-[#f5f5f5]"} z-10`}>
        ...
        >
          ...
          <div className="flex-1 overflow-auto">
            {isLoading ? (
              ...
              <div className="p-4 text-center text-muted-foreground">No folders open. Click "Add Folder" to get started.</div>
            ) : (
              <FileExplorer data={fileTree} selectedFiles={selectedFiles} onSelectionChange={setSelectedFiles} />
            )}
          </div>
        </div>
